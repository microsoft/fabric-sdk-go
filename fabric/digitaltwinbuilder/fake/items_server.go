...
func (i *ItemsServerTransport) dispatchDeleteDigitalTwinBuilder(req *http.Request) (*http.Response, error) {
	...
	var options *digitaltwinbuilder.ItemsClientDeleteDigitalTwinBuilderOptions
	if hardDeleteParam != nil {
		options = &digitaltwinbuilder.ItemsClientDeleteDigitalTwinBuilderOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteDigitalTwinBuilder(req.Context(), workspaceIDParam, digitaltwinbuilderIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
