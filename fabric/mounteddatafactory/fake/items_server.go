...
func (i *ItemsServerTransport) dispatchDeleteMountedDataFactory(req *http.Request) (*http.Response, error) {
	...
	var options *mounteddatafactory.ItemsClientDeleteMountedDataFactoryOptions
	if hardDeleteParam != nil {
		options = &mounteddatafactory.ItemsClientDeleteMountedDataFactoryOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteMountedDataFactory(req.Context(), workspaceIDParam, mountedDataFactoryIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
