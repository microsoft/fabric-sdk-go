...
func (i *ItemsServerTransport) dispatchDeleteEventstream(req *http.Request) (*http.Response, error) {
	...
	var options *eventstream.ItemsClientDeleteEventstreamOptions
	if hardDeleteParam != nil {
		options = &eventstream.ItemsClientDeleteEventstreamOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteEventstream(req.Context(), workspaceIDParam, eventstreamIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
