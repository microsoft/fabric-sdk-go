...
func (i *ItemsServerTransport) dispatchDeleteVariableLibrary(req *http.Request) (*http.Response, error) {
	...
	var options *variablelibrary.ItemsClientDeleteVariableLibraryOptions
	if hardDeleteParam != nil {
		options = &variablelibrary.ItemsClientDeleteVariableLibraryOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteVariableLibrary(req.Context(), workspaceIDParam, variableLibraryIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
