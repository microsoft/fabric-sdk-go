...
func (i *ItemsServerTransport) dispatchDeleteMLModel(req *http.Request) (*http.Response, error) {
	...
	var options *mlmodel.ItemsClientDeleteMLModelOptions
	if hardDeleteParam != nil {
		options = &mlmodel.ItemsClientDeleteMLModelOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteMLModel(req.Context(), workspaceIDParam, mlModelIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
