...
func (i *ItemsServerTransport) dispatchDeleteMirroredAzureDatabricksCatalog(req *http.Request) (*http.Response, error) {
	...
	var options *mirroredazuredatabrickscatalog.ItemsClientDeleteMirroredAzureDatabricksCatalogOptions
	if hardDeleteParam != nil {
		options = &mirroredazuredatabrickscatalog.ItemsClientDeleteMirroredAzureDatabricksCatalogOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteMirroredAzureDatabricksCatalog(req.Context(), workspaceIDParam, mirroredAzureDatabricksCatalogIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
...
func (i *ItemsServerTransport) dispatchBeginGetMirroredAzureDatabricksCatalogDefinition(req *http.Request) (*http.Response, error) {
	...
	if beginGetMirroredAzureDatabricksCatalogDefinition == nil {
		...
		var options *mirroredazuredatabrickscatalog.ItemsClientBeginGetMirroredAzureDatabricksCatalogDefinitionOptions
		if formatParam != nil {
			options = &mirroredazuredatabrickscatalog.ItemsClientBeginGetMirroredAzureDatabricksCatalogDefinitionOptions{
				Format: formatParam,
			}
		}
		respr, errRespr := i.srv.BeginGetMirroredAzureDatabricksCatalogDefinition(req.Context(), workspaceIDParam, mirroredAzureDatabricksCatalogIDParam, options)
		if respErr := server.GetError(errRespr, req); respErr != nil {
			return nil, respErr
		}
