...
func (i *ItemsServerTransport) dispatchDeleteReflex(req *http.Request) (*http.Response, error) {
	...
	var options *reflex.ItemsClientDeleteReflexOptions
	if hardDeleteParam != nil {
		options = &reflex.ItemsClientDeleteReflexOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteReflex(req.Context(), workspaceIDParam, reflexIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
