...
func (i *ItemsServerTransport) dispatchDeleteReport(req *http.Request) (*http.Response, error) {
	...
	var options *report.ItemsClientDeleteReportOptions
	if hardDeleteParam != nil {
		options = &report.ItemsClientDeleteReportOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteReport(req.Context(), workspaceIDParam, reportIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
