...
func (testsuite *FakeTestSuite) TestItems_ListOntologies() {
	...
	pager := client.NewListOntologiesPager(exampleWorkspaceID, &ontology.ItemsClientListOntologiesOptions{Recursive: nil,
		RootFolderID:      nil,
...
func (testsuite *FakeTestSuite) TestItems_DeleteOntology() {
	...
	client := testsuite.clientFactory.NewItemsClient()
	_, err = client.DeleteOntology(ctx, exampleWorkspaceID, exampleOntologyID, &ontology.ItemsClientDeleteOntologyOptions{HardDelete: nil})
	testsuite.Require().NoError(err, "Failed to get result for example ")

	// From example
	ctx = runtime.WithHTTPHeader(testsuite.ctx, map[string][]string{
		"example-id": {"Hard delete an ontology example"},
	})
	exampleWorkspaceID = "cfafbeb1-8037-4d0c-896e-a46fb27ff229"
	exampleOntologyID = "5b218778-e7a5-4d73-8187-f10824047715"
	...
	_, err = client.DeleteOntology(ctx, exampleWorkspaceID, exampleOntologyID, &ontology.ItemsClientDeleteOntologyOptions{HardDelete: to.Ptr(true)})
	testsuite.Require().NoError(err, "Failed to get result for example ")
}
