...
func (i *ItemsServerTransport) dispatchDeleteApacheAirflowJob(req *http.Request) (*http.Response, error) {
	...
	var options *apacheairflowjob.ItemsClientDeleteApacheAirflowJobOptions
	if hardDeleteParam != nil {
		options = &apacheairflowjob.ItemsClientDeleteApacheAirflowJobOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteApacheAirflowJob(req.Context(), workspaceIDParam, apacheAirflowJobIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
