...
func (i *ItemsServerTransport) dispatchDeleteDataPipeline(req *http.Request) (*http.Response, error) {
	...
	var options *datapipeline.ItemsClientDeleteDataPipelineOptions
	if hardDeleteParam != nil {
		options = &datapipeline.ItemsClientDeleteDataPipelineOptions{
			HardDelete: hardDeleteParam,
		}
	}
	respr, errRespr := i.srv.DeleteDataPipeline(req.Context(), workspaceIDParam, dataPipelineIDParam, options)
	if respErr := server.GetError(errRespr, req); respErr != nil {
		return nil, respErr
	}
